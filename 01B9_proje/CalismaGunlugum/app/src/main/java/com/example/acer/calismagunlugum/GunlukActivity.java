package com.example.acer.calismagunlugum;
...
public class GunlukActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        add.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (eroll_no.getText().toString().trim().length() == 0 ||
                    ...
                    showMessage("HATA", "Lütfen Tüm Değerleri Giriniz");
                    return;
                }
                db.execSQL("INSERT INTO student VALUES('" + eroll_no.getText() + "','" + ename.getText() +
                        "','" + emarks.getText() + "');");
                showMessage("İŞLEM BAŞARILI", "Kayıt Başarıyla Eklendi");
                clearText();
            }
        });
        delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                if (eroll_no.getText().toString().trim().length() == 0) {
                    showMessage("HATA", "Silmek İstediğiniz Tarihi Giriniz");
                    return;
                }
                ...
                if (c.moveToFirst()) {
                    ...
                    showMessage("İŞLEM BAŞARILI", "Kayıt Silindi");
                } else {
                    showMessage("HATA", "Geçersiz Tarih!");
                }
                clearText();
            }
        });
        modify.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                if (eroll_no.getText().toString().trim().length() == 0) {
                    showMessage("HATA", "Düzenlemek İstediğiniz Tarihi Giriniz");
                    return;
                }
                Cursor c = db.rawQuery("SELECT * FROM student WHERE rollno='" + eroll_no.getText() + "'", null);
                if (c.moveToFirst()) {
                    ...
                    showMessage("İŞLEM BAŞARILI", "Kayıt Başarıyla Düzenlendi");
                } else {
                    showMessage("HATA", "Geçersiz Tarih!");
                }
                clearText();
        ...
        view.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                if (eroll_no.getText().toString().trim().length() == 0) {
                    showMessage("HATA", "Görüntülemek İstediğiniz Tarihi Giriniz");
                    return;
                }
                Cursor c = db.rawQuery("SELECT * FROM student WHERE rollno='" + eroll_no.getText() + "'", null);
                if (c.moveToFirst()) {
                    ...
                    showMessage("HATA", "Girilen Tarihe Ait Kayıt Bulunamadı!");
                    clearText();
                }
            }
        });
        viewall.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                Cursor c = db.rawQuery("SELECT * FROM student ORDER BY rollno DESC", null);
                if (c.getCount() == 0) {
                    showMessage("HATA", "Kayıt Bulunamadı");
                    return;
                }
                StringBuffer buffer = new StringBuffer();
                while (c.moveToNext()) {
                    buffer.append("Tarih: " + c.getString(0) + "\n");
                    buffer.append("Ders: " + c.getString(1) + "\n");
                    buffer.append("Çözülen Soru Sayısı: " + c.getString(2) + "\n\n");
                }
                showMessage("---GÜNLÜĞÜM---", buffer.toString());
            }
        });
